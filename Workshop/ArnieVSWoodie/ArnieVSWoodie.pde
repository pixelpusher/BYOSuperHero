...
PImage bodyTex, headTex, armTex, legTex, headTex2, bg;

String bodyTexFile = "FieryMarioBody.png";
String headTexFile = "face.png";
String headTexFile2 = "woody.png";
...
bg = loadImage("9FitnessCenter.jpeg");
  ...
  fireballTex = new GLTexture(this, "Pills.png");
...
void draw()
{
  ...
  { 
    ...
    if (skel.calibrated)
    {
      ...
      PVector ElbowShoulderDir = PVector.sub(skel.rightElbowPos, skel.rightShoulderPos );
      PVector ElbowShoulderDir2 = PVector.sub(skel.leftElbowPos, skel.leftShoulderPos );
      float transformangle = atan2(ElbowShoulderDir.x, ElbowShoulderDir2.x);
...
if ( yangle < -0.9 )
      ...
      renderRectFromVectors(skel.facePos, skel.neckPos, 100, 50, headTex);}
else {
      renderRectFromVectors(skel.facePos, skel.neckPos, 100, 50, headTex2);
}
...
void newSwarm(GLTexture tex, PVector startPos, PVector direction)
{
  ...
  if (timeDiff > 300)
  {
    ...
    int numFireballs = int( random(1, 2) );
