...
class ImageParticleSwarm
{
  ...
  // 
  // make a GL model from a PVector List (can be ArrayList)
  // 


  boolean makeModel(List<PVector> verts)
  {
    ...
    if (verts != null && verts.size() > 0)
    {
      ...
      for (PVector v : verts) 
      {
        glmodel.updateVertex(index, v.x, v.y, v.z);
        ++index;
      }
    ...
    if (mAlive)
    {
      ...
      else
      {
        ...
        for (int n = 0; n < glmodel.getSize(); ++n) {
          ...
          // process vert...
          particleBehaviour.updateVertex(n, vert);
